...
DEBUG: bool = False
RUN_SCRIPT_INTERVAL_HOURS: int = 6
...
def initialize_repo() -> Repo:
    ...
    Returns:
        Repo: Initialized git repository object.
    """
    try:
        repo = Repo(REPO_PATH)
...
def check_remote_changes(repo: Repo) -> bool:
    """
    ...
    Returns:
        bool: True if remote changes are detected, False otherwise.
    """
    ...
    logging.info(f"Remote changes detected: {has_changes}")
    return has_changes
...
def load_input_file(file_path: str) -> list:
    ...
    Returns:
        list: List of addresses loaded from the file.
    """
    try:
        with open(file_path, "r") as file:
            addresses = [line.strip() for line in file]
            logging.info(f"Loaded {len(addresses)} addresses from {file_path}")
            return addresses
...
def load_existing_output(output_file_path: str) -> dict:
    """
    ...
    Returns:
        dict: Dictionary with subnets as keys and comments as values.
    """
    existing_entries = {}
        ...
        with open(output_file_path, "r", encoding="utf-8") as file:
            for line in file:
                if line.strip() and not line.startswith("#"):
                    ...
                    existing_entries[entry] = comment
        logging.info(f"Loaded {len(existing_entries)} entries from {output_file_path}")
    else:
...
def remove_duplicates(addresses: list) -> list:
    ...
    Returns:
        list: List of unique addresses sorted.
    """
...
def group_into_subnets(addresses: list) -> list:
    """
    ...
    Returns:
        list: List of grouped IP addresses and subnets.
    """
...
def sort_subnets(entries: list) -> list:
    """
    Sort the subnets in CIDR notation from largest to smallest subnets and after that, the IPs based on all four octets.

    Args:
        entries (list): List of subnets and IP addresses.

    Returns:
        list: Sorted list of subnets and IP addresses.
    """
...
def check_whois_info(entries: list, existing_entries: dict) -> dict:
    """
    Check Whois information for each entry that is new or changed.

    Args:
        entries (list): List of subnets.
        existing_entries (dict): Dictionary with existing subnets and comments.

    Returns:
        dict: Dictionary with subnets as keys and Whois information as values.
    """
    info_dict = {}
    logging.info(
    ...
    for entry in entries:
        if entry not in existing_entries:
            try:
                ...
                info_dict[entry] = comment
            except Exception as e:
                logging.error(f"Error processing {entry}: {e}")
        else:
            info_dict[entry] = existing_entries[entry]
    logging.info("WhoIs information processed.")
    return info_dict


def write_output_to_file(
    new_entries: dict, existing_entries: dict, output_file_path: str
) -> bool:
    """
    Write the processed addresses to the output file and return success.

    Args:
        new_entries (dict): Dictionary with new subnets and Whois information.
        existing_entries (dict): Dictionary with existing subnets and comments.
        output_file_path (str): Path to the output file.
    ...
    try:
        with open(output_file_path, "w", encoding="utf-8") as file:
            for entry, comment in all_entries.items():
                comment = (
                    (
        ...
        if file_size > max_size_bytes:
            ...
            logging.info(f"Output written to {output_file_path} ({file_size} bytes)")
            return True
    except IOError as e:
...
def process_input(file_to_process: str) -> bool:
    """
    Process the input file: load, deduplicate, validate, group, and write to output.

    Args:
        file_to_process (str): Name of the file to process.

    Returns:
        bool: True if writing output was successful, False otherwise.
    """
    raw_addresses = load_input_file(os.path.join(REPO_PATH, file_to_process))
    unique_addresses = remove_duplicates(raw_addresses)
    valid_addresses = validate_addresses(unique_addresses)
    grouped_addresses = group_into_subnets(valid_addresses)
    sorted_addresses = sort_subnets(grouped_addresses)

    output_file_path = os.path.join(REPO_PATH, "output")
    if "automated" in file_to_process:
        output_file_path = os.path.join(output_file_path, "blocklist-industrial.txt")
    elif "manual" or "raw" in file_to_process:
        output_file_path = os.path.join(
            output_file_path, "blocklist-industrial-manual.txt"
        )

    existing_entries = load_existing_output(output_file_path)
    looked_up_addresses = check_whois_info(sorted_addresses, existing_entries)
    ...
    return write_output_to_file(looked_up_addresses, existing_entries, output_file_path)
...
def main() -> None:
    ...
    if check_remote_changes(repo) or DEBUG:
        pull_changes(repo)

        for file in INPUT_FILES_TO_PROCESS:
            processed = process_input(file)

        if processed and not DEBUG:
            commit_and_push_changes(repo)
        else:
            logging.error(
                "Error during output file validation or in debug mode, not pushing to git."
            )
    else:
        logging.info(
